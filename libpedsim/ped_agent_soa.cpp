...
// init values
#pragma omp parallel for shared(agents, xs, ys, desiredXs, desiredYs, \
                                waypoints, currs) num_threads(8)      \
    schedule(static)
    for (size_t i = 0; i < size; i++) {
        ...
        // float x = nextDest->getx(), y = nextDest->gety(), r =
        // nextDest->getr(); destXs[i] = x; destYs[i] = y; destRs[i] = r;
